...
### RIGHT NOW IT'S IN wptview/utilities/
BASE_PATH = abspath(join(split(__file__)[0], pardir))
assert exists(BASE_PATH)
...
trailing_whitespace_check = Check(trailing_whitespace_function, [BASE_PATH])
trailing_whitespace_check.excluded_names = ["/lf_scripts/", "/utilities"]
trailing_whitespace_check.included_filetypes = [".js", ".css", ".html", ".md"]
