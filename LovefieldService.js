...
LovefieldService.prototype.buildSchema_ = function() {
  ...
  schemaBuilder.createTable('comments').
      ...
      addColumn('result_id', lf.Type.INTEGER).
      addColumn('run_id', lf.Type.INTEGER).
      addUnique("unique_fk", ['result_id', 'run_id']).
...
LovefieldService.prototype.selectComment = function(result_id, run_id) {
  comments = this.comments;
  return this.db_.select(comments.comment).
    from(comments).
    where(lf.op.and(
            comments.result_id.eq(result_id),
            comments.run_id.eq(run_id))).
    exec();
}

LovefieldService.prototype.insertComment = function(result_id, run_id, comment) {
  comments = this.comments;
  return this.db_.insert().
    into(comments).
    values([comments.createRow({'result_id':result_id, 'run_id':run_id, 'comment': comment})]).
    exec();
}

LovefieldService.prototype.updateComment = function(result_id, run_id, comment) {
  comments = this.comments;
...
LovefieldService.prototype.deleteComment = function(result_id, run_id) {
  comments = this.comments;
...
LovefieldService.prototype.deleteComments = function(run_id) {
  ...
  q2 = db_.select(test_results.result_id)
    .from(test_results);
